...
description: 'Adding, editing and deleting contact properties'
---
...
| ---------------- | ------------------------- | ------------- |
| Email            | _hi@example.co_           | `{email}`     |
| First Name       | _Chris_                   | `{firstName}` |
| Last Name        | _Frantz_                  | `{lastName}`  |
| Notes            | _Favorite color is blue._ | `{notes}`     |
| Source           | _API_                     | `{source}`    |
| Subscribed       | `true`                    | N/A           |
| User Group       | Investors                 | `{userGroup}` |
| User Id          | _ask523236_               | `{userId}`    |
